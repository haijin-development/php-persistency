...
$spec->describe( "A subclass of Expression_Visitor", function() {
    ...
    $this->let( "expression_visitor", function() {
        return new Expressions_Visitor_Subclass();
    });
    ...
    $this->it( 'raises an Unexpected_Expression_Error for all expressions by default', function() {

        foreach( $this->all_expressions as $each_expression ) {
            $this->expect( function() use($each_expression){

                $this->expression_visitor->visit( $each_expression );

            }) ->to() ->raise(
                Unexpected_Expression_Error::class,
                function($error) use($each_expression) {

                    $expression_name = get_class( $each_expression );

                    $this->expect( $error->getMessage() ) ->to() ->equal(
                        "Unexpected {$expression_name}."
                    );

                    $this->expect( $error->get_expression() ) ->to() ->be() ->a(
                        Expression::class
                    );
...
class Expressions_Visitor_Subclass extends Expression_Visitor
{
}